...
class HybridRecommender:
    ...
    def get_onboarding_movies(self, n=10):
        """Most-rated movies to show on the onboarding screen."""
    ...
    def recommend_content_from_ratings(self, ratings_dict, n=5):
        """Content recs built from a temporary {movie_id: rating} dict (onboarding)."""
    ...
    def recommend_content(self, user_id, n=5):
    ...
    def recommend_hybrid(self, user_id, n=10):
        profile = self._build_user_profile(user_id)
        if profile is None:
            return self.recommend_svd(user_id, n)
        scores = cosine_similarity(profile, self.tfidf_matrix).flatten()
    ...
    def recommend_svd(self, user_id, n=10):
...
def router(user_id, n, recommender):
    stage = get_user_stage(user_id, recommender.train_df)

    if stage in ("new", "near-cold"):
        return stage, {
            "popular": recommender.recommend_popular(n=n),
            "content": recommender.recommend_content(user_id, n=n),
        }
    elif stage == "early":
        return stage, recommender.recommend_hybrid(user_id, n=n)
    else:
        return stage, recommender.recommend_svd(user_id, n=n)
...
def render_rec_list(recs, key_prefix):
    """Render a list of recs with 👍/👎 buttons."""
    ...
    if st.button("Generate Recommendations", type="primary"):
        ...
        if stage == "new":
            ...
            with st.spinner("Finding your recommendations..."):
                ...
                _, results = router(uid, n_recs, rec)
            st.session_state.results = results
            st.session_state.feedback = {}
            st.session_state.stage = 'results'
        st.rerun()

# Stage: onboarding (new users only)
    ...
    with col_submit:
        if st.button("Get my recommendations", type="primary", disabled=len(ratings) == 0):
            with st.spinner("Building your profile..."):
                content = rec.recommend_content_from_ratings(ratings, n=st.session_state.n_recs)
                popular_recs = rec.recommend_popular(n=st.session_state.n_recs)
